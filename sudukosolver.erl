...
UpdatedGrids = update_maps(Grids, Cell, []),
 ...
 SolutionPairs = pair_evaluation(Pairs, UpdatedGrids, PairGridOptions, []),
...
pair_evaluation([],_,_,Acc) -> Acc;
pair_evaluation([{{CellX, CellY},V,F}, {{PairCellX,PairCellY},PV,PF}|Rest], Grids, GridOptions, Acc) ->
 ...
 PairCellList = lists:filter(fun(X) -> X /= PV end,cell_solution_values(GridOptions,{{PairCellX, PairCellY},PV,PF}, [])),

 DirectionGrids = get_direction_grids(Grids, Direction, {CellX, CellY}, get_grid_centre(CellX), get_grid_centre(CellY), []),

 UniqueOption = get_single_option(CellList, DirectionGrids, 0),
 UniquePairOption = get_single_option(PairCellList, DirectionGrids, 0),

%%logic is wrong, as its not checking all the combinations and therefore updates

 io:format("{~w,~w} unique pair count ~w versus {~w,~w} option for value ~w ~w v ~w ~n", [CellX, CellY, UniquePairOption, PairCellX, PairCellY, UniqueOption, V, PV]),

 if UniquePairOption /= 0, UniqueOption == 0 -> 
      pair_evaluation(Rest, Grids,GridOptions,[{{CellX, CellY},V,"true"}|Acc]);
    UniquePairOption == 0, UniqueOption /= 0 ->  
      pair_evaluation(Rest, Grids, GridOptions, [{{PairCellX,PairCellY},PV,"true"}|Acc]);
    true -> pair_evaluation(Rest, Grids,GridOptions, Acc)
 end.

get_single_option([], _, UniqueOption) -> UniqueOption;

get_single_option([Option|T], Grids, UniqueOption) ->
 %%io:format("option ~w ~n", [hd(Option)]),
 CountOptions = count_options(Grids, Option, 0),
 if CountOptions == 1 -> get_single_option(T, Grids, Option);
  true -> get_single_option(T, Grids, 0)
 end.

count_options([], _, Count) -> Count;
count_options([Grid|Tail], Option, Count) ->
 Res = lists:any(fun({{X,Y},V,F}) -> V == Option end, Grid),

 if Res == true -> count_options(Tail, Option, Count);
  true -> count_options(Tail, Option, Count + 1)
 end.


get_direction({{PairCellX, PairCellY},_,_}, {{CellX, CellY},_,_}) ->
...
cell_solution_values([{{GX,GY},GV}|T],{{X,Y},V,F},Acc) ->
 if GX == X, GY == Y -> cell_solution_values([],{{X,Y},V,F}, grid_solution_values([{{GX,GY},GV}], Acc));
